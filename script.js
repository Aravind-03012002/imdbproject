...
async function fetchMovies(category, containerId) {
    const url = `https://www.omdbapi.com/?s=${category}&page=1&apikey=${key}`;
    const res = await fetch(url);
    const data = await res.json();
...
function displayMovies(movies, containerId) {
    ...
    movies.forEach(movie => {
        let imgSrc = movie.Poster !== 'N/A' ? movie.Poster : 'img/blank-poster.webp';
        ...
        <div class="fav-item">
            <div class="fav-poster">
            <a href="movie.html?id=${movie.imdbID}"><img src=${imgSrc} alt="Favourites Poster"></a>
            </div>
            <div class="fav-details">
                <div class="fav-details-box">
                    <div>
                        <p class="fav-movie-name"><a href="movie.html?id=${movie.imdbID}">${movie.Title}</a></p>
                        <p class="fav-movie-rating"><a href="movie.html?id=${movie.imdbID}">${movie.Year}</a></p>
                    </div>
                    <div>
                       <i class="fa-solid fa-bookmark" style="cursor:pointer;" onClick=addTofavorites('${movie.imdbID}')></i>
                   </div>
                    </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    fetchMovies('telugu', 'top10Movies');
    fetchMovies('hindi','hindiMovies')
    fetchMovies('avengers', 'popularMovies');
    fetchMovies('spiderman', 'trendingMovies');
    fetchMovies('horror','horrorShows')
    fetchMovies('cartoon','cartoonShows');
    fetchMovies('tamil','tamilMovies');
    fetchMovies('webseries','webseries')
});
